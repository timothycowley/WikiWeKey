...
/**
 * This Class identifies and interprets Markdown italic tags ("_" type) and renders them as HTML
 *
 * @author Tim
 *
 */
public class ItalicUnderscoreMark extends EmphasisMark {

  private static final String TYPE = "italicUnderscore";
  private static final String START_REGEX = "^[^_]_[^ _]";
  private static final String END_REGEX = "^[^ _]_[^_]";
  private static final String BOTH_REGEX = "^[^ _]_[^ _]";
   ...
   *
   */
  public ItalicUnderscoreMark(final int position, final String kind) {
    super(position, kind);
    setMarkType(TYPE);
  ...
  public List<Mark> readMarkdown(final String currentSubstring, final List<Mark> totalMarkList,
    ...
    if (bothMatcher.find(0)) {
      final ItalicUnderscoreMark newMark = new ItalicUnderscoreMark(i + 1, BOTHKIND);
      totalMarkList.add(newMark);
    } else if (startMatcher.find(0)) {
      final ItalicUnderscoreMark newMark = new ItalicUnderscoreMark(i + 1, STARTKIND);
      totalMarkList.add(newMark);
    } else if (endMatcher.find(0)) {
      final ItalicUnderscoreMark newMark = new ItalicUnderscoreMark(i + 1, ENDKIND);
      totalMarkList.add(newMark);
    }
